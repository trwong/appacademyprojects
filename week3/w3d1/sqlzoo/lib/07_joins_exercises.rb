...
def ford_supporting_films
  ...
  JOIN movies
    ON movies.id = castings.movie_id
  WHERE actors.name = 'Harrison Ford' AND
    castings.ord != 1
  SQL
end
...
def films_and_stars_from_sixty_two
  ...
  execute(<<-SQL)
  SELECT movies.title, actors.name FROM actors
  JOIN castings
    ON actors.id = castings.actor_id
  JOIN movies
    ON movies.id = castings.movie_id
...
def travoltas_busiest_years
  ...
  execute(<<-SQL)
  SELECT movies.yr, COUNT(*) FROM movies
  JOIN castings
    ON movies.id = castings.movie_id
  JOIN actors
    ON actors.id = castings.actor_id
...
def films_by_cast_size
  ...
  execute(<<-SQL)
  SELECT movies.title, COUNT(actors) AS num_actors FROM movies
